...
public abstract class AbstractAutoProxyCreator extends ProxyProcessorSupport
	...
	/**
	 * 如有必要，即如果它有资格被代理，则包装给定的 bean。
	 *
	 * @param bean     原始bean实例
	 * @param beanName bean的名称
	 * @param cacheKey 元数据访问的缓存键
	 * @return 包装 bean 的代理，或原样的原始 bean 实例
	 */
	protected Object wrapIfNecessary(Object bean, String beanName, Object cacheKey) {
		...
		//isInfrastructureClass判断是否存在Advices、Advisors 和 AopInfrastructureBeans三个注解
		//shouldSkip判断是否是创建原始实例，如果两个条件都满足，这不生成代理对象
	...
	/**
	 * 判断指定的类是否需要生成代理
	 * 默认实现将 Advices、Advisors 和 AopInfrastructureBeans 视为基础设施类。如果存在以上的类则需要生成代理对象
	 *
	...
	/**
	 * 如果满足以下条件，子类应重写此方法以返回 {@code true}
	 * 给定的 bean 不应被考虑用于此后处理器的自动代理。
	 * <p>有时我们需要能够避免这种情况的发生，例如，如果它会导致
	 * 循环引用或者是否需要保留现有目标实例。
	 * 此实现返回 {@code false} 除非 bean 名称指示
	 * 根据 {@code AutowireCapableBeanFactory} 约定的“原始实例”。
	 *
	 * @param beanClass bean的类
	 * @param beanName  bean的名称
	 * @return 是否跳过给定的bean
	 * @参见 org.springframework.beans.factory.config.AutowireCapableBeanFactory#ORIGINAL_INSTANCE_SUFFIX
	 */
